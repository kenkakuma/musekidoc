...
const glazeTechniques = JSON.parse(
  fs.readFileSync(path.join(process.cwd(), 'glaze-techniques.json'), 'utf-8')
);
const relatedArtists = JSON.parse(
  fs.readFileSync(path.join(process.cwd(), 'related-artists-detail.json'), 'utf-8')
);
...
async function main() {
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(process.cwd(), 'import-task01-report.json'),
